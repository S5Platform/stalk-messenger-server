...
var _getChannel = function(data) {
  return new Promise( (resolve, reject) => {
    var cQuery = new Parse.Query(Channels);
    if( data.channelKey ){
      cQuery.equalTo("channelKey", data.channelKey);
    } else {
      cQuery.containsAll("users", data.users);
    }

    return cQuery.find().then(
        (channels) => {
          ...
          if( channels ){
            ...
            for( var inx = 0 ; inx < channels.length ; inx++ ){
              if( data.channelKey == channels[inx].get("channelKey") || ( channels[inx].get("users") && ( channels[inx].get("users").length == data.users.length ) ) ){
