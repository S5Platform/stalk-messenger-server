...
var SessionServer = exports.SessionServer = function (options, cb) {
  ...
  var parseArgs = {
    ...
    appName: options.appName ||"S5Messenger",
    ...
    serverURL: options.serverURL || 'http://localhost:8080/parse', //options.host+':'+options.port+'/parse'
    publicServerURL: options.publicServerURL || 'http://localhost:8080/parse' //options.host+':'+options.port+'/parse'
  };

  if( options.verifyUserEmails ) parseArgs['verifyUserEmails'] = options.verifyUserEmails;
  if( options.emailAdapter ) parseArgs['emailAdapter'] = options.emailAdapter;
