...
var SessionServer = exports.SessionServer = function (options, cb) {
  ...
  var parseArgs = {
    ...
    masterKey: options.master || '1q2w3e', //Add your master key here. Keep it secret!
    // ** 주의 ** cloud 에서도 사용하는 URL 에는 반드시 protocol 이 명시되어야 함.
  ...
  if( options.filesAdapter && options.filesAdapter.module == "parse-server-s3-adapter") {

    var s3Adapter = new S3Adapter(options.filesAdapter.options);
    parseArgs['filesAdapter'] = s3Adapter;

    console.log('\n * \'filesAdapter\' is added. * \n');
    console.log(options.filesAdapter);
